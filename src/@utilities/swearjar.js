...
const swearjar = {
    ...
    scan(text, callback) {
        ...
        while ((match = regex.exec(text)) != null) {
            ...
            if (
                key in this._badWords.simple &&
                Array.isArray(this._badWords.simple[key])
            ) {
                if (
                    callback(
                        word,
                        match.index,
                        this._badWords.simple[key],
                        key,
                        'simple',
                    ) === false
                ) {
                ...
                key in this._badWords.emoji &&
                Array.isArray(this._badWords.emoji[key])
            ) {
                if (
                    callback(
                        word,
                        match.index,
                        this._badWords.emoji[key],
                        key,
                        'emoji',
                    ) === false
                ) {
                ...
                if (word) {
                    if (
                        callback(
                            word,
                            match.index,
                            this._badWords.regex[regexString],
                            regexString,
                            'regex',
                        ) === false
                    ) {
    ...
    addRegex(word, categories) {
        ...
        if (word in this._badWords.regex) {
            this._badWords.regex[word] = this._badWords.regex[word].concat(
                categoryArray,
            );
        } else {
    ...
    addSimple(word, categories) {
        ...
        if (word in this._badWords.simple) {
            this._badWords.simple[word] = this._badWords.simple[word].concat(
                categoryArray,
            );
        } else {
    ...
    addEmoji(word, categories) {
        ...
        if (word in this._badWords.emoji) {
            this._badWords.emoji[word] = this._badWords.emoji[word].concat(
                categoryArray,
            );
        } else {
...
swearjar._badWords = require('./swearjar-en_US.json');
