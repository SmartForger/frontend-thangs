...
const swearjar = {
  ...
  scan(text, callback) {
    ...
    while ((match = regex.exec(text)) != null) {
      ...
      if (key in this._badWords.simple && Array.isArray(this._badWords.simple[key])) {
        if (
          callback(word, match.index, this._badWords.simple[key], key, 'simple') === false
        ) {
          break
        }
      } else if (
        ...
        if (
          callback(word, match.index, this._badWords.emoji[key], key, 'emoji') === false
        ) {
          break
        }
      } else {
  ...
  profane(text) {
    ...
    this.scan(text, (_word, _index, _categories) => {
      profane = true
  ...
  censor(text) {
    ...
    this.scan(text, (word, index, _categories) => {
      censored =
